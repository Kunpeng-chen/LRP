...
)
{
    ...
    lrp_write_u16_le(&out[LRP_HEADER_SIZE + frame->payload_len], crc);
...
)
{
    ...
    expected_len = (uint16_t)(LRP_HEADER_SIZE + payload_len + LRP_CRC_SIZE);

    if (len != expected_len) {
        return LRP_ERR_INVALID_LENGTH;
    }

    received_crc = lrp_read_u16_le(&data[LRP_HEADER_SIZE + payload_len]);

    computed_crc = lrp_crc16_ccitt(data, (uint16_t)(LRP_HEADER_SIZE + payload_len));
